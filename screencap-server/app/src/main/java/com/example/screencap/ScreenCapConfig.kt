...
class ScreenCapConfig {
    data class Config(
        ...
        val skipFrames: Int = 1,
        val targetPriority: List<Int> = listOf(0, 1, 2),
    ...
    fun update(cropSize: Int, frameRate: Int, bitrate: Int, modelPath: String,
               confThreshold: Float, iouThreshold: Float, detectBody: Boolean, detectHead: Boolean,
               lineThickness: Int = 2, skipFrames: Int = 1, targetPriority: List<Int> = listOf(0, 1, 2),
               sizeTolerance: Float = 0.15f) {
        config = Config(cropSize, frameRate, bitrate, modelPath, confThreshold, iouThreshold,
                       detectBody, detectHead, lineThickness, skipFrames, targetPriority, sizeTolerance)
        val json = JSONObject().apply {
            ...
            put("skip_frames", skipFrames)
            put("target_priority", JSONArray(targetPriority))
            put("size_tolerance", sizeTolerance)
        }.toString(2)
        try {
            configDir.mkdirs()
    ...
    private fun loadConfig() {
        try {
            ...
            val priorityArray = json.optJSONArray("target_priority")
            val priorityList = if (priorityArray != null) {
                (0 until priorityArray.length()).map { priorityArray.getInt(it) }.toList()
            } else {
                listOf(0, 1, 2)
            }
            config = Config(
                ...
                skipFrames = json.optInt("skip_frames", 1),
                targetPriority = priorityList,
                sizeTolerance = json.optDouble("size_tolerance", 0.15).toFloat()
            )
